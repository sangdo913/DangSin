...
void lineBall(Vector3 &start, Vector3 &end, Vector3 &reflect_point, Vector3 ball) {
	...
	tempdir.normalize();
	reflect_point = d1 >= RADIUS ? start + ((end - start).normalize()*(sqrt(D*D - d1*d1) -
		sqrt(4 * RADIUS*RADIUS - d1*d1))) : start + ((end - start).normalize()*(sqrt(D*D - d1*d1) - sqrt(RADIUS*RADIUS - d1*d1)-RADIUS));
	reflect_point = reflect_point.distance(ball) < 2*RADIUS ? ball + (reflect_point - ball).normalize()*2*RADIUS*1.01f : reflect_point;

	if (reflect_point.distance(ball) > 2 * RADIUS)
	{
		printf("수정됨\n");
		reflect_point = ball + (reflect_point - ball).normalize() * 2 * RADIUS*0.999f;
	}
...
bool planEq(Vector3 start, Vector3 ball, Vector3 X) {//공과 시작점의 벡터를 법선벡터로하는 평면의 방정식.X가 평면위인지 아래인지 확인.
	...
	Vector3 ap = X - start;
	if (((n.x*ap.x) + (n.y*ap.y)) > 0) return true;//0보다 크면1
	else return false;
}
void lineTrace()
{
	...
	if (start_point.distance(yellowBall.transform->position) <= RADIUS) {
		for (int i = 0; i < 2; i++) {
			...
			cout << "b2 : " << b2 << endl;
			if ((d1 < (2.0 * RADIUS)) && (planEq(start, redBall.transform->position, end_point) == b1)&&(start.distance(redBall.transform->position)>2*RADIUS)) {//빨간공과 부딪친다면
			...
			else if ((d2 < (2 * RADIUS)) && (planEq(start, whiteBall.transform->position, end_point) == b2)
				&& (start.distance(whiteBall.transform->position)>2 * RADIUS)) {//흰공과 부딪친다면
	...
	/*흰공을 움직인다면*/
		...
		for (int i = 0; i < 5; i++) {
			...
			if ((d1 < (2 * RADIUS)) && (planEq(start, redBall.transform->position, end_point) == b1) && (start.distance(redBall.transform->position)>2 * RADIUS)) {//빨간공과 부딪친다면(반대쪽 처리해야함.)
				cout << "redball" << endl;
				lineBall(start, line_end, reflect_point[count_reflect], redBall.transform->position);
			}
			else if ((d2 < (2 * RADIUS)) && (planEq(start, yellowBall.transform->position, end_point) == b2)
				&& (start.distance(yellowBall.transform->position)>2 * RADIUS)) {//노란공과 부딪친다면
				cout << "yball" << endl;
